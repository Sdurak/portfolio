...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 bg-gradient-to-b from-gray-800 to-black text-white">
      <div>
        <h1 className="text-5xl font-signature ml-2">Segah Durak</h1>
      </div>
      ...
      <div className="flex justify-center">
        <button
          onClick={langHandler}
          className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
        >
          <span>{textData.lang}</span>
          {lang === "tr" ? textDataTr.language : textDataEng.language}
        </button>
      </div>
    </div>
  );
